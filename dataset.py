...
class BiofilmDataset(Dataset):
    def __init__(self, image_dir, mask_dir, colored_mask_dir, augmentation=False, mode="train"):
        ...
        image_paths = [os.path.join(image_dir, f) for f in listdir(image_dir) if isfile(join(image_dir, f) and 'patch' in f)]
...
def leaveOneSEMimageOut(imgPath = None, maskPath = None, outputBaseDir = None, imgNum = None):
    
    testImageSourceDir = os.path.join(outputBaseDir, f"{imgNum}-BSE-testSourceDir", "images")
    testMaskSourceDir = os.path.join(outputBaseDir, f"{imgNum}-BSE-testSourceDir", "colored-masks")

    os.makedirs(testImageSourceDir, exist_ok=True)
    os.makedirs(testMaskSourceDir, exist_ok=True)

    newImgPath = os.path.join(testImageSourceDir, os.path.basename(imgPath))
    if os.path.exists(newImgPath):
        os.remove(newImgPath)
    newMaskPath = os.path.join(testMaskSourceDir, os.path.basename(maskPath))
    if os.path.exists(newMaskPath):
        os.remove(newMaskPath)
